...
Snapshot freshness is computed without materializing `payload` or `inline_search` JSON. Runtime diagnostics join the in-scope rules to `rule_search_snapshots` and read only aggregate/count values and `fetched_at`. The regression test deliberately corrupts one `inline_search` JSON value and verifies freshness telemetry still succeeds. This preserves the corruption isolation established by the F-01 repair: malformed legacy snapshot JSON cannot break the freshness check itself.

The scope matches scheduled fetch behavior: `enabled` includes only enabled rules; `all` includes manually disabled rules; completion-auto-disabled movie/Jellyfin rules are excluded in either case. A snapshot is stale after two configured schedule intervals. A rule with no snapshot becomes an overdue-missing failure only after the rule itself is older than that same freshness window; a newer rule without a snapshot is reported as `pending_snapshots` and does not create a transient F-02 failure. The two-interval window is deliberately conservative to avoid treating one slightly late run or a just-created rule as stale data; the five-minute watchdog still promotes a continuing violation after three observations.
...
- after any completed scheduled run, one or more in-scope snapshots are older than two configured intervals, or an in-scope rule older than the same window still has no snapshot -> `FAIL` with `effectiveness_state=stale_snapshots` and bounded counts/timestamps;
- recent rules that are still awaiting their first snapshot remain visible as pending telemetry but do not fail F-02;
