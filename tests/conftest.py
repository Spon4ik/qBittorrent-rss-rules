...
def pytest_configure(config: pytest.Config) -> None:
    ...
    _ORIGINAL_TEST_ENV = {
        key: value for key, value in os.environ.items() if key.startswith(_TEST_ENV_PREFIX)
    }
    for key in _ORIGINAL_TEST_ENV:
...
def pytest_unconfigure(config: pytest.Config) -> None:
    ...
    for key in tuple(os.environ):
        if key.startswith(_TEST_ENV_PREFIX):
            os.environ.pop(key, None)
...
@pytest.fixture(autouse=True)
def guard_test_database_path(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    from isolation_guard import assert_test_database_is_temporary

    from app import config, db

    database_path = tmp_path / "pytest.db"
    ...
    def create_isolated_engine(database_url: str, *args: object, **kwargs: object):
        assert_test_database_is_temporary(database_url)
        return create_engine(database_url, *args, **kwargs)

    monkeypatch.setattr(db, "create_engine", create_isolated_engine)
    yield
    config.get_environment_settings.cache_clear()
    db.reset_db_caches()
