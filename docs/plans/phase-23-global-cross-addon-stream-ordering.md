...
| P23-02 | Implement qB-side row-attribution and series-precision precursors. | Codex | 2026-03-30 | completed | qB rows are more descriptive, the main qB RSS Jackett path carries series episode-floor precision before broad fallback, and season-finale `S(next)E00` floors survive edit-form round trips. | Landed in `app/services/jackett.py`, `app/routes/pages.py`, `app/services/stremio_addon.py`, and `app/schemas.py`; 2026-04-02 follow-up fixed `start_episode=0` edit-form rendering in `app/routes/pages.py` and added route/browser QA regression coverage in `tests/test_routes.py` and `scripts/closeout_browser_qa.py`; validated with focused pytest/ruff/mypy plus closeout QA reruns. |
