...
- Existing-rule reconciliation follow-up on 2026-08-07 is implemented and live-validated for `v1.4.6`: each active unseen Stremio-linked rule is restored to the configured enabled and managed-quality defaults during ordinary sync, including historical rules, while shared completion evidence remains authoritative for watched-title auto-disablement. The full gate passes with `526` tests, WinUI builds with zero warnings/errors, Docker `/health` serves `v1.4.6`, and the final read-only live audit found `307` active Stremio titles, `245` unseen titles expected enabled, zero persisted state/profile mismatches, and zero qB enabled-rule mismatches.
