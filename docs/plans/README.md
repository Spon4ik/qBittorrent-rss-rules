...
`ROADMAP.md` remains the high-level product roadmap. The files in `docs/plans/` are the detailed source of truth for work that is actively being implemented or prepared. Phases 5 through 12 are implemented/release-validated, and no new active implementation phase is open yet after the `v0.7.0` release closeout.
