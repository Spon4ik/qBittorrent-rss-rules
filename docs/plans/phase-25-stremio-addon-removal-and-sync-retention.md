...
| P25-04 | Cut and publish the breaking-change release. | Codex | 2026-04-19 | completed | Version is bumped consistently, changelog/docs are updated, and the release commit/tag are published to git. | `v1.1.2` already exists for the taxonomy/profile patch, so the final post-R8 closeout was version-synced to `1.1.3`; PR #10 merged, tag `v1.1.3` pushed, and the GitHub Release is published. |
...
1. Select the next post-`v1.1.3` backlog phase before starting new implementation.
