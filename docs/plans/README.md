...
- `docs/plans/phase-20-stremio-library-rule-sync.md`: implemented and release-validated `v0.8.0` plan covering local Stremio discovery, authoritative library sync, Stremio-managed rule creation/linkage, background auto-sync, and the native Stremio addon parity slice
...
- `docs/plans/phase-23-global-cross-addon-stream-ordering.md`: historical phase plan for the shipped native-addon aggregation slice that is now being retired from this repo
...
- `docs/plans/phase-25-stremio-addon-removal-and-sync-retention.md`: active breaking-change plan covering removal of the native Stremio addon while retaining Stremio sync/watch-progress integration

`ROADMAP.md` remains the high-level product roadmap. The files in `docs/plans/` are the detailed source of truth for work that is actively being implemented or prepared. Phases 5 through 24 are implemented historical work. Phase 25 is the active sync-only Stremio boundary-cleanup slice.
