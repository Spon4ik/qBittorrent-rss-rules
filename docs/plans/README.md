...
- `docs/plans/phase-5-media-aware-rule-form-and-metadata.md`: active phase 5 validation and closeout plan
- `docs/plans/phase-6-jackett-active-search.md`: active next-release implementation plan for Jackett-backed active search

`ROADMAP.md` remains the high-level product roadmap. The files in `docs/plans/` are the detailed source of truth for work that is actively being implemented or prepared, with phase 5 still in validation and phase 6 now underway as the next-release implementation slice.
