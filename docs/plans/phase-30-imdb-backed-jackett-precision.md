...
- Browser/grouping follow-up on 2026-08-07: rule `69bbdd53-03c9-4f2e-b8cf-4becf93e225f` (`Ridiculous`, IMDb `tt41376508`) exposed a server/browser identity-parity regression. The backend correctly kept `The Ridiculous 6` hidden, but browser-side recomputation demoted a raw primary row to fallback and then accepted its loose token match. IMDb-backed browser filtering now requires the same strict title identity used by the backend when no matching result IMDb ID exists. Literal same-title rows such as the three `Ghosts` titles remain eligible unless a conflicting IMDb ID or configured release year disproves them; text alone cannot safely distinguish identical names. Group merging now also retains every variant-specific tracker details URL and renders labeled links such as `Details (Kinozal)` and `Details (RuTor)` while queue/magnet actions remain group-level. Released as `v1.4.9` through PR `#31`.
...
- Scheduled-refresh stale-snapshot follow-up on 2026-08-04: the daily enabled-rule run executed but completed only `126/253`, leaving Silo and 126 other failed rules on their 2026-08-02 snapshots. A live selected refresh reproduced an immediate fatal `Selected Jackett indexers do not advertise IMDb-enforced search` result: scoped direct search propagated the capability diagnostic before exact-title fallback could run. Scoped IMDb-first search now records that state as a warning and continues into exact-title fallback while preserving the rule that saved direct indexer scope never searches aggregate `all`. The focused regression passes; the full gate passes with Ruff/mypy clean and `521 passed`; WinUI builds with zero warnings/errors; Docker `/health` serves `v1.4.3`; a deployed Silo refresh advanced the snapshot from `2026-08-02 08:24:05 UTC` to `2026-08-04 19:08:38 UTC` with `75` fetched rows and `4` matches; and the subsequent live enabled-rule schedule completed `253/253` successfully in 27m39s, replacing the previous `126 succeeded / 127 failed` result. Silo advanced again to `19:36:23 UTC`, and the next daily run is scheduled for `2026-08-05 19:36:30 UTC`. PR `#22`, tag `v1.4.3`, and the GitHub Release are published.
- Managed 1080p floor follow-up on 2026-08-06: the current Ghosts snapshot exposed `Ghosts - S1-2E1-40 ... WEBRip 720p` as visible and queueable because the built-in `At Least Full HD` preset started at the generic `HD` taxonomy rank and included both `HD` and `720p`. The preset now starts at `Full HD`, excludes explicit 720p and lower resolutions while keeping generic `HD` neutral for valid `HD 1080p` labels, migrates settings that match the former deployed token set, and leaves custom/manual profiles untouched. The exact Ghosts regressions pass; the full gate is green with `524` tests; WinUI builds with zero warnings/errors; Docker serves `v1.4.4`; post-JavaScript browser state keeps both reported Ghosts packs hidden without Queue actions; and live qB readback reports zero 720p matches after the Ghosts rule sync. PR `#24`, tag `v1.4.4`, and the GitHub Release are published.
