...
- Phase 40 provider-isolated settings and Stremio auth discovery is implemented,
  ...
  all `355` rules, and Docker health is green. PR `#37` is merged; annotated tag
  `v1.4.12` is pushed; and the GitHub Release is published.
...
- Phase 38 quota-safe Jackett feed validation is implemented and release-validated
  ...
  feed while synchronizing qB RSS rules; health validation now stops after HTTP
  success and XML parsing. This prevents private-tracker download quotas such as
  Kinozal's daily 10-file allowance from being consumed without an explicit
  Queue action. Focused sync, Queue, and qB client coverage passes (`47 passed`),
  including a regression proving a `Subnautica 2` RSS enclosure is never
  requested; the full gate passes with Ruff/mypy clean and all `531` tests green;
  WinUI builds with zero warnings/errors; Docker `/health` serves `v1.4.10`; and
  a live enabled Kinozal-backed rule sync succeeded while Jackett's Kinozal
  torrent-download error count remained `0 -> 0`. PR `#33` is merged; annotated
  tag `v1.4.10` is pushed; and the GitHub Release is published at
  `https://github.com/Spon4ik/qBittorrent-rss-rules/releases/tag/v1.4.10`.
- `v1.4.9` is published from merged PR `#31`: IMDb-backed browser filtering now mirrors backend strict title identity so `The Ridiculous 6` cannot become visible for `tt41376508` merely through the token `Ridiculous`; genuinely identical titles remain eligible unless IMDb/year evidence distinguishes them. Grouped Jackett results now preserve and render every variant-specific tracker details URL with tracker labels, including RuTor, instead of exposing only the representative row's details link. Ruff/mypy and all `530` tests pass; WinUI builds with zero warnings/errors; Docker `/health` serves `v1.4.9`; a forced live `Ridiculous` snapshot refresh persisted multi-tracker detail metadata; and post-JavaScript browser QA found zero visible `Ridiculous` rows while rendering grouped links such as `Details (BigFANGroup)` and `Details (RuTor)`. Tag and GitHub Release `v1.4.9` are published.
- Phase 37 qB recovery and new-rule bootstrap is implemented, validated, and published as `v1.4.8` (`docs/plans/phase-37-qb-recovery-and-new-rule-bootstrap.md`). Temporary qB transport failures are detected by a lightweight recovery scheduler and requeued immediately after qB becomes reachable, while configuration/authentication errors are not retry-looped; new-rule creation now queues both qB sync and a deduplicated initial forced snapshot fetch, including disabled rules. `cmd.exe /c scripts\check.bat` passes with Ruff/mypy clean and all `529` tests green; WinUI builds with zero warnings/errors; the shared Docker backend rebuilt successfully; `/health` serves `status=ok` / `app_version=1.4.8`; and an in-container recovery tick completed cleanly with zero currently failed transport candidates. PR `#30` is merged and tag/GitHub Release `v1.4.8` are published.
- Jellyfin movie-completion classification is corrected and live-validated as the `v1.4.7` release candidate. Live `tt0790636` (`Dallas Buyers Club`) evidence showed Stremio fully unseen and the selected Jellyfin user row at `Played=0`, `PlayCount=1`, with only `49,170,959` playback ticks, but `_movie_is_completed()` treated PlayCount alone as completed and left the RSS rule disabled with `movie_completion_sources=["jellyfin"]`. Movie completion now requires Jellyfin `Played=1`, matching the existing watch-progress contract and Jellyfin UI watched mark. The full gate passes with Ruff/mypy clean and all `527` tests green; WinUI builds with zero warnings/errors; Docker `/health` serves `v1.4.7`; normal Jellyfin/Stremio sync completed with zero errors; and live readback now proves Stremio incomplete, Jellyfin `Played=0`, RSS enabled on managed `2160p_hdr`, completion evidence cleared, and qB present/enabled. A library-wide read-only check found zero remaining movie rules carrying false Jellyfin completion evidence.
- Stremio unseen-rule reconciliation is implemented and live-validated as the `v1.4.6` release candidate. Live inspection found active historical Stremio-linked rules that remained disabled or on Plain/older quality profiles despite having no completion evidence. Ordinary Stremio sync now restores every active unseen linked rule to the configured enabled and managed-quality defaults while leaving shared completion-auto-disabled titles untouched. The full gate passes with Ruff/mypy clean and all `526` tests green; WinUI builds with zero warnings/errors; Docker `/health` serves `v1.4.6`; and the deployed sync reconciled the full active library. Final read-only Stremio/persistence/qB audit found `307` active titles, `245` unseen titles expected enabled, zero persisted state/profile mismatches, and zero qB enabled-rule mismatches.
- Jellyfin missing-episode watch-progress handling is implemented, live-validated, and published as `v1.4.5` on 2026-08-07. The reported sync (`13` matches, `12` writes, `2` errors) was reproduced against live provider state as Stremio progress for `tt14688458:S02E10` with no exact Jellyfin episode item available for a safe user-data write. Missing exact Jellyfin episode targets now count as skips rather than provider errors, and other exact episodes in the same series group continue syncing. The full gate passes with Ruff/mypy clean and all `525` tests green; WinUI builds with zero warnings/errors; Docker `/health` serves `v1.4.5`; and a deployed live sync completed with `13` matches, `41` writes, `11` skips, and zero errors while explicitly reporting S02E10 as skipped. PR `#26` is merged; annotated tag `v1.4.5` is pushed; and the GitHub Release is published at `https://github.com/Spon4ik/qBittorrent-rss-rules/releases/tag/v1.4.5`.
- Managed 1080p quality-floor hardening is implemented, live-validated, and published as `v1.4.4` on 2026-08-06. The reported Ghosts Kinozal row (`Ghosts - S1-2E1-40 ... WEBRip 720p`) was visible because the built-in `At Least Full HD` preset incorrectly started at the generic `HD` taxonomy rank and therefore explicitly included `HD` and `720p`. The preset now starts at `Full HD`, includes only 1080p-or-higher resolution aliases, and excludes explicit 720p and lower resolutions; generic `HD` remains neutral so valid labels such as `HD 1080p` are not vetoed. Existing settings that match the former deployed preset migrate automatically; custom/manual profiles remain unchanged. The full gate passes with Ruff/mypy clean and all `524` tests green; WinUI builds with zero warnings/errors; Docker `/health` serves `v1.4.4`; the deployed Ghosts card remains hidden and non-queueable after JavaScript recomputation; and qB readback has no positive HD/720p alternative, explicitly rejects 720p, and returns zero current 720p matching articles for Ghosts. PR `#24` is merged; annotated tag `v1.4.4` is pushed; and the GitHub Release is published at `https://github.com/Spon4ik/qBittorrent-rss-rules/releases/tag/v1.4.4`.
- Scheduled snapshot refresh stale-result repair is implemented, live-validated, and published as `v1.4.3` on 2026-08-04. Live state proved the scheduler was enabled and ran at `12:25:48 UTC`, but completed partially (`126` succeeded, `127` failed), leaving Silo on its `2026-08-02 08:24:05 UTC` snapshot. A live selected fetch reproduced the root cause: when none of the saved direct indexers advertised IMDb-enforced series search, the capability diagnostic escaped as a fatal Jackett error before exact-title fallback. Scoped IMDb-first search now retains that diagnostic as a warning and continues to direct exact-title fallback without querying aggregate `all`. The full gate passes with Ruff/mypy clean and all `521` tests green; WinUI builds with zero warnings/errors; Docker `/health` serves `v1.4.3`; and a deployed Silo refresh advanced the snapshot to `2026-08-04 19:08:38 UTC` with `75` fetched rows and `4` matches. The subsequent full enabled-rule schedule completed `253/253` successfully in 27m39s, advanced Silo again to `19:36:23 UTC`, persisted `last_status=ok`, and scheduled the next daily run for `2026-08-05 19:36:30 UTC`. PR `#22` is merged; annotated tag `v1.4.3` is pushed; and the GitHub Release is published at `https://github.com/Spon4ik/qBittorrent-rss-rules/releases/tag/v1.4.3`.
- Managed quality-filter consistency is repaired, live-validated, and published as `v1.4.2` on 2026-08-04. Live diagnosis of the reported `Star Wars: The Mandalorian and Grogu` rule found that the persisted snapshot correctly marked both seleZen 720p/WEBRip rows hidden, but the saved global `1080p` and `2160p_hdr` managed-profile definitions were both empty. The edit page resolved those empty settings, rendered every quality control off, and its browser recomputation promoted the backend-hidden rows to `Visible` with Queue actions. Empty non-Plain managed presets now restore the current taxonomy defaults and `SettingsService.get_or_create()` persists the repair, while genuinely unfiltered behavior remains available through the explicit Plain profile. The full gate passes with Ruff/mypy clean and all 520 tests green; the WinUI build passes with zero warnings/errors; Docker `/health` returns `v1.4.2`; and a live Playwright probe confirms both reported rows remain hidden after JavaScript recomputation with `Missing required quality tags.` and zero visible cards. PR `#21` is merged; annotated tag `v1.4.2` is pushed; and the GitHub Release is published at `https://github.com/Spon4ik/qBittorrent-rss-rules/releases/tag/v1.4.2`.
- Jellyfin write-back routing, write contract, provider identity, and manual title authority are repaired and published as `v1.4.1` on 2026-08-03. Live diagnosis advanced the sole watch-progress error (`tt6663582`) through four concrete layers: container loopback caused connection refused; the saved stale key caused 401 after routing to `host.docker.internal:8096`; the authenticated session-progress request caused 400 because no playback session existed; and the first user-data request exposed that Stremio's IMDb ID must be replaced by the matched Jellyfin internal item ID. Shared Compose now supplies the Docker-safe URL and existing active Jellyfin key, while write-back uses Jellyfin's `UserItems/{itemId}/UserData` API with the selected user and matched Jellyfin item IDs. Automatic Stremio library sync no longer overwrites `Rule.content_name` or `Rule.normalized_title`, while an explicit Metadata Lookup refreshes both fields. Focused regressions and Compose validation pass; the full gate passes with Ruff/mypy clean and all 518 tests green; the WinUI build passes with zero warnings/errors; Docker `/health` returns `v1.4.1`; and two concurrent live startup paths each completed watch-progress sync successfully with `12` matches, `41` writes, and zero errors. The automatic Stremio sync kept all 306 existing titles unchanged, confirming the title-authority fix in the deployed runtime. PR `#20` merged to `main` at `94da79c2`; annotated tag `v1.4.1` is pushed; and the GitHub Release is published at `https://github.com/Spon4ik/qBittorrent-rss-rules/releases/tag/v1.4.1`.
- Phase 36 Real-Debrid search and qBittorrent HTTP acceleration is published as `v1.4.0` on 2026-08-03. PR `#19` merged to `main` at `4f0874dc`; annotated tag `v1.4.0` is pushed; and the GitHub Release is published at `https://github.com/Spon4ik/qBittorrent-rss-rules/releases/tag/v1.4.0`. Ruff, mypy, all 517 tests, the zero-warning WinUI build, shared Docker rebuild, and `/health` are green. A live 341-rule sync completed successfully with zero enabled-rule errors; all 254 locally enabled/app-managed remote rules now carry the `qb-rss-rules` RSS `torrentParams.tags` array, while one unrelated remote-only rule remains intentionally untagged. Live account inspection found Real-Debrid and MyJDownloader disconnected, so authenticated OAuth/web-seed/fallback smoke tests remain the documented post-connection verification step.
- Saved-indexer Jackett search scope authority is implemented, live-validated, and published as `v1.3.4` on 2026-08-03. Explicit `Rule.search_indexers` scope now searches only the selected direct Jackett endpoints for IMDb-first, precise-title, broad-fallback, and standard searches; it no longer sends movie/title searches through the aggregate `all` endpoint before applying a local scope filter. Per-indexer Torznab failures such as `[100] Invalid API Key` remain warnings when another selected tracker succeeds. The reported live failure also exposed a stale app-saved Jackett API key; the saved credential was synchronized to Jackett's active key without exposing either value. `cmd.exe /c scripts\check.bat` passed (`491 passed`, `302 warnings`) with Ruff and mypy clean; the WinUI desktop build passed with zero warnings/errors; the shared Docker backend rebuilt healthy on `v1.3.4`; and a forced live Grogu refresh completed in 39.2 seconds, advanced the snapshot to `2026-08-03 13:08:51 UTC`, persisted 2 fetched rows from `seleZen`, and contained no `indexer=all` movie/title failure. Three direct-indexer failures remained correctly isolated as warnings while the working selected tracker result was preserved. PR `#18` merged to `main` at `da7a1d91`; annotated tag `v1.3.4` is pushed; and the GitHub Release is published at `https://github.com/Spon4ik/qBittorrent-rss-rules/releases/tag/v1.3.4`.
- Stremio movie unwatch recovery is repaired, live-validated, and published as `v1.3.3` on 2026-08-03. Live diagnosis of `Star Wars: The Mandalorian and Grogu` (`tt30825738`) found a stale Stremio movie state with only 72 seconds watched but `flaggedWatched=1` and a non-empty `watched` marker, so rule sync correctly retained Stremio completion evidence and kept the rule disabled. The inverse Stremio movie write-back path was incomplete: an unwatched/in-progress Jellyfin winner updated position but did not clear `flaggedWatched`, `timesWatched`, or `watched`. Movie write-back now clears all three completion markers when `completed=False`, with a regression using the exact live stale-state shape. The focused Stremio/watch-progress/watch-state/routes slice passed; `cmd.exe /c scripts\check.bat` passed (`490 passed`, `302 warnings`) with Ruff and mypy clean; the WinUI desktop build passed with zero warnings/errors; the shared Docker backend rebuilt healthy on `v1.3.3`; and the targeted live repair preserved 72-second position while clearing all completion markers. The subsequent Stremio rule sync re-enabled Grogu, cleared its completion source/auto-disable flag, and recreated the enabled qB rule; a separate pre-existing watch-progress write to `tt6663582` still warns with connection refused and is unrelated to Grogu recovery. PR `#17` merged to `main` at `adb11629`; annotated tag `v1.3.3` is pushed; and the GitHub Release is published at `https://github.com/Spon4ik/qBittorrent-rss-rules/releases/tag/v1.3.3`.
- RuTracker direct-title completeness is repaired, versioned as the `v1.3.1` patch, and live-validated on 2026-08-02 for rule `59d66ba9-0616-473a-b968-43eaa4a78c3a` (`Stuart Fails to Save the Universe`). RuTracker returns zero rows for the IMDb-enforced request but three rows for the same direct title request; the app previously stopped precise-title recovery after Jackett's aggregate `all` endpoint returned rows from other trackers, then its scope filter also rejected RuTracker's Torznab display label `RuTracker.org` because the saved indexer slug is `rutracker`. Precise-title recovery now keeps the aggregate response and runs the explicit selected-indexer group as a bounded parallel completeness pass; standard fallback direct batches use the same bounded concurrency; and scope filtering compares normalized indexer-key variants. The focused Jackett suite and full project gate passed (`489 passed`, `302 warnings`) with Ruff and mypy clean; the WinUI desktop rebuilt with zero warnings/errors; the shared Docker backend rebuilt successfully and is healthy; `/health` returned `status=ok` / `app_version=1.3.1`; and a forced live refresh grew the saved snapshot from 9 to 16 fetched rows with all three `RuTracker.org` variants visible, including topic `6887212`.
- The consolidated `v1.3.0` release is published on 2026-08-01: PR `#14` merged to `main` at `c53f1fcf`, annotated tag `v1.3.0` is pushed, and the GitHub Release is available at `https://github.com/Spon4ik/qBittorrent-rss-rules/releases/tag/v1.3.0`. Version touchpoints are synchronized across `pyproject.toml`, FastAPI `/health`, route assertions, and the WinUI backend compatibility constant; `cmd.exe /c scripts\check.bat` passed (`488 passed`, `302 warnings`) with Ruff and mypy clean; `scripts\run_dev.bat desktop-build` passed with zero warnings/errors; the shared Docker backend rebuilt successfully; and `/health` returned `status=ok` / `app_version=1.3.0`.
- Manual saved-snapshot refresh is repaired and live-validated on 2026-08-01: rules-grid magnifier/card search links now include `refresh_snapshot=1` whenever a snapshot exists instead of silently replaying the saved snapshot; IMDb-capable per-indexer recovery probes run concurrently; precise-title and broad-fallback recovery stop after the first query variant that returns rows instead of repeating slow equivalent variants; and an explicit Fetch selected action now permits a user-selected completion-auto-disabled rule while automatic Fetch all continues excluding completed titles. The reported `Star Wars: The Mandalorian and Grogu` snapshot advanced from `2026-07-11 08:57:34 UTC` to `2026-07-31 23:10:46 UTC` through the live refresh route, then a deployed Fetch selected call completed `1/1` successfully with 41 fetched rows and advanced it again to `2026-07-31 23:18:25 UTC`. Focused Jackett/routes/rule-fetch regressions passed; the full project gate passed (`488 passed`, `302 warnings`); the final shared Docker rebuild passed; and `/health` returned `status=ok` / `app_version=1.2.18`.
- Rule-fetch/search start feedback is repaired on 2026-08-01: Fetch all and Fetch selected now show their status in the always-visible utility strip and immediately switch the global operation console to a starting state; row search and edit-page run/refresh snapshot actions provide the same immediate feedback while their navigation request is pending. Parallel Jackett batches now advance progress as each rule finishes while preserving API result order, instead of remaining at `0/N` until the whole executor completes. Focused static, route, and rule-fetch regressions passed (`187 passed`); the full project gate passed (`487 passed`, `302 warnings`); the shared Docker backend rebuilt successfully; `/health` returned `status=ok` / `app_version=1.2.18`; browser smoke confirmed immediate `Background work starting` feedback for Fetch all and row search plus a visible `Running Jackett fetch...` page status; and a live selected-rule API call registered `jackett_fetch` at `0/1` while the provider request was pending.
- Series-pack/new-episode and Jackett partial-failure hardening is implemented and Docker-validated on 2026-07-31: qB generated regexes no longer treat a multi-episode range as an existing single episode, saved-snapshot/backend/browser filters keep packs such as `Silo S03E01-04` when `S03E04` is new regardless of `Keep unseen`, and explicit packs containing only library episodes remain hidden. Multi-indexer Jackett fan-out now treats timeout, connection, unsupported-query, and HTTP failures as per-indexer warnings whenever another selected tracker succeeds; warnings identify the indexer and no longer persist API-key-bearing URLs. qB rule-sync feed health checks now run with bounded parallelism and a five-second per-request timeout so multiple down trackers cannot serialize a rule update. Validation evidence: focused regressions passed; full `cmd.exe /c scripts\check.bat` passed (`487 passed`, `302 warnings`); the shared Docker Compose service rebuilt successfully; `/health` returned `status=ok` / `app_version=1.2.18`; a live Silo snapshot retained 2 of 145 results, including both `S03E01-04` packs while rejecting standalone existing `S03E03`; and a live single-rule qB sync completed successfully with zero errors in 8.4 seconds despite unhealthy selected feeds.
- Live Jellyfin multi-user selection was repaired on 2026-07-23: the configured Jellyfin DB contains `Liam` and `Spon4ik`, while the app setting was blank and automatic sync correctly stopped with `Multiple Jellyfin users were found`. The saved `jellyfin_user_name` is now `Spon4ik`; a read-only connection test selected that user, and a forced Docker sync completed with 0 Jellyfin rule errors (`0 updated`, `21 unchanged`, `311 skipped`) plus `10` watch-progress matches / `47` watch-progress writes. Docker `/health` remained healthy on `v1.2.18`.
- Phase 33 UI regression follow-up is implemented and Docker/browser validated as `v1.2.18` on 2026-07-14: inline rule and advanced search results now render search notices as one bounded region; selecting `Show hidden fetched rows` replaces the filtered-empty panel with the actual result table and reports the hidden-row state; and the desktop workspace uses a resilient flex layout with a guaranteed 10rem table viewport plus internal scrolling instead of collapsing the table after verbose warnings. Live Docker validation against the reported Fauda rule confirmed 63 hidden rows become visible, the empty state is hidden, and the table remains usable; focused route/static checks, JavaScript syntax, and touched-file Ruff passed. The shared Docker backend has been rebuilt and `/health` returns `status=ok` / `app_version=1.2.18`.
- Phase 35 smart audiobook rule search is implemented and locally/Docker validated on 2026-06-20 (`docs/plans/phase-35-smart-audiobook-rule-search.md`): audiobook metadata lookup now defaults to `Smart audiobook`, which tries Google Books before OpenLibrary and reports combined provider failures when all providers miss; audiobook rule forms persist structured title/author/publisher/genre/ISBN hints in `Rule.search_metadata`; saved audiobook rule searches now build `JackettSearchRequest` with those hints plus ISBN; and Jackett book searches run capability-aware direct `t=book` indexer probes before deterministic generic variants (`title author`, `title`, final `isbn`) while keeping qB RSS rule generation title-based. Validation evidence: red-first focused regressions passed; `tests/test_metadata.py` passed (`16 passed`); `tests/test_jackett.py` passed (`59 passed`); `tests/test_routes.py` passed; `tests/test_static_assets.py` passed (`6 passed`); `node --check app/static/app.js` passed; touched-file Ruff passed; full `cmd.exe /c scripts\check.bat` passed (`485 passed`, `302 warnings`); shared Docker Compose rebuild passed; PowerShell `Invoke-WebRequest` hit the known local `NullReferenceException`, and `curl.exe http://127.0.0.1:8000/health` returned `status=ok` / `app_version=1.2.17`.
- Rules workbench release-signal waterfall filtering is implemented and validated on 2026-06-04 as a targeted Phase 29 follow-up: the main Release signal column now renders compact `e/v/r` counts (`exact visible / visible / results fetched`), card view mirrors the same count, and the Release signal dropdown now filters by waterfall thresholds (`Any results`, `Visible results`, `Exact results`, `No snapshot`) while preserving existing release-signal sorting. Validation evidence: the new release-signal route regressions failed red before the implementation and passed after it; `tests/test_routes.py` passed; `node --check app/static/app.js` passed; full `cmd.exe /c scripts\check.bat` passed (`473 passed`, `292 warnings`); shared Docker Compose rebuild passed; PowerShell `Invoke-WebRequest` hit the known local `NullReferenceException`, and `curl.exe http://127.0.0.1:8000/health` returned `status=ok` / `app_version=1.2.17`.
- qB duplicate torrent queue handling is fixed and Docker-validated on 2026-06-03: qBittorrent `409 Conflict` responses from `/api/v2/torrents/add` are now treated as idempotent duplicate-add success for both URL/magnet and uploaded `.torrent` handoffs, preventing an already-present torrent from surfacing as a hard `qBittorrent request failed` error on rule pages such as `5207fbac-b5bc-4591-b114-2ea5052a0766`. Other qB endpoints still reject unexpected `409`s unless explicitly allowed. Validation evidence: duplicate-add regressions failed red before the fix and passed after it; `tests/test_qbittorrent_client.py` passed (`17 passed`); `tests/test_routes.py -k "queue_search_result_api or test_qb or feed_urls"` passed (`16 passed`, `32 warnings`); `tests/test_selective_queue.py` passed (`16 passed`); Ruff passed on touched files; shared Docker Compose rebuild passed; Docker `/health` returned `status=ok` / `app_version=1.2.17`.
- Automatic Jellyfin sync transient SQLite-lock handling is implemented and validated on 2026-06-02 as a Phase 32 follow-up: when the background Jellyfin worker hits a SQLAlchemy `OperationalError` containing `database is locked` while reading/reusing the app settings row, it now treats the tick as retryable contention, clears the observed Jellyfin DB marker, and backs off to the next poll instead of persisting a misleading `Automatic Jellyfin sync failed` status. Real non-lock sync errors still record the existing error status/message. Validation evidence: the new auto-sync regression failed red before the fix, then passed; `tests/test_jellyfin_auto_sync.py` passed (`3 passed`); `tests/test_jellyfin.py tests/test_jellyfin_auto_sync.py` passed (`30 passed`); Ruff passed on touched files; full `cmd.exe /c scripts\check.bat` passed (`471 passed`, `292 warnings`); shared Docker Compose rebuild passed; PowerShell `Invoke-WebRequest` hit the known local `NullReferenceException`, and `curl.exe http://127.0.0.1:8000/health` returned `status=ok` / `app_version=1.2.17`.
- Phase 34 continuing-series follow-up is implemented and locally/Docker validated as the `v1.2.17` Death in Paradise repair on 2026-06-02: Cinemeta `status=Continuing`, scheduled videos, and open-ended `releaseInfo` / `year` values now prevent and clear finished-series watch-state auto-disable even when the latest known episode has been watched and no next season is listed yet. This makes the cleanup/re-enable mechanism safer for still-open shows such as Death in Paradise. Validation evidence: focused metadata/Jellyfin/Stremio tests and Ruff passed for the continuing-series patch; full `cmd.exe /c scripts\check.bat` passed (`470 passed`, `292 warnings`) after a one-off order-sensitive sync-queue assertion passed on rerun; desktop build passed (`0 Warning(s)`, `0 Error(s)`); shared Docker Compose rebuild passed; Docker `/health` returned `status=ok` / `app_version=1.2.17`; Docker Cinemeta probe for `tt1888075` returned season 15 known/released episodes 1-8 and `ended False`; Docker Stremio sync re-evaluated rule `7d534f3d-ec7e-4bdf-ab16-5c994871a00c`; Docker qB sync recreated/synced it successfully; the rule now has `enabled=True`, `movie_completion_auto_disabled=False`, empty completion sources, and `last_sync_status=OK`.
- Phase 34 quota-safe series re-enablement is implemented and locally/Docker validated as the `v1.2.16` follow-up on 2026-06-02 (`docs/plans/phase-34-quota-safe-series-reenablement.md`): finished-series auto-disable now uses a shared Cinemeta-first series catalog lookup for known/released episode inventory by IMDb ID and season, with OMDb only as an allowed fallback, so background Jellyfin/Stremio sync can avoid OMDb quota while still recognizing future episodes and revived seasons. Jellyfin and Stremio now share the same completion proof, missing catalog evidence no longer disables a series from local library data alone, and Stremio re-enable/disable outcomes take priority over ordinary floor updates. Validation evidence: focused metadata/Jellyfin/Stremio/watch-state/sync/rule-fetch/routes slice passed; Ruff passed for touched backend/test files; full `cmd.exe /c scripts\check.bat` passed (`468 passed`, `292 warnings`); desktop build passed (`0 Warning(s)`, `0 Error(s)`); shared Docker Compose rebuild passed; Docker `/health` returned `status=ok` / `app_version=1.2.16` via `curl.exe` after PowerShell `Invoke-WebRequest` hit the known local `NullReferenceException`.
- Watched-completion qB cleanup is implemented and Docker-validated on 2026-06-02 as a backend follow-up to the Phase 32 watch-state work: completed movie rules and conservatively proven finished-series rules now auto-disable through the shared watch-state path, disabled completion rules are excluded from Jackett fetch batches even when disabled rules are included, disabled rules remove their synced or same-name qB RSS rule instead of recreating a disabled remote payload, and the app mirrors qBittorrent desktop's "Remove unused categories" behavior globally through the Web API by removing every qB category with zero torrents after cleanup. The finished-series proof now distinguishes known episodes from released episodes, so planned/future later episodes such as a known S01E08 keep a rule enabled even if S01E07 is watched. The cleanup does not delete torrents, files, tags, feeds, or save paths. Validation evidence: focused qB/sync/Jellyfin/Stremio/watch-state/rule-fetch tests passed; Ruff passed for touched backend/test files; full `cmd.exe /c scripts\check.bat` passed (`463 passed`, `292 warnings`); shared Docker Compose rebuild passed; Docker `/health` returned `status=ok` / `app_version=1.2.15` via `curl.exe`.
- Phase 33 web UI consistency refresh is implemented and Docker/browser validated as the `v1.2.15` compact rules workspace follow-up on 2026-06-02 (`docs/plans/phase-33-web-ui-consistency-refresh.md`): after production QA showed the prior `v1.2.9` pass had traded scroll reduction for unreadable clipped sections, the latest patch replaces the full-width qB diagnostics band with a compact disclosure, moves verbose rule checkbox explanations into accessible help text, keeps the rule settings rail readable, and turns Unified Query Results source/scope detail into compact disclosures instead of clipped or oversized text blocks. The latest feedback correction makes Rule settings non-collapsible, combines Start season / Start episode into one row, attaches metadata lookup and quality-authority explanations to the actual related controls through hover/focus help, hides the desktop edit-page Unified Query Results summary to return height to the table, and constrains desktop edit pages so scrolling stays inside the settings rail or result table rather than the document. A follow-up QA miss was corrected after the prior gate measured only high-level rail/results overlap and missed child overflow between `Quality authority` and `Criteria / Core Identity`; the rule form now stacks both panels full-width in the narrow settings rail, and targeted rendered metrics confirm no parent or descendant overlap. Rendered Docker metrics at 1600x900 confirm no document-level scroll or horizontal overflow, no Rule settings/results overlap, old drawer markup absent, and long-result table scrolling inside `.search-table-wrap` with usable table height. The watch-state floor and category/media filtering fixes remain in place: watched `S05E09` advances to `S05E10`, relevant `Hacks S5E01-10` TV rows remain visible, and unrelated Books/tutorial Hacks rows stay hidden. Backend APIs, data attributes, and database schema are unchanged. Validation evidence: `node --check app/static/app.js` passed; focused route/static checks passed; full `cmd.exe /c scripts\check.bat` passed (`450 passed`, `292 warnings`); desktop build passed (`0 Warning(s)`, `0 Error(s)`); shared Docker Compose rebuild passed; Docker `/health` returned `status=ok` / `app_version=1.2.15`; Docker-served screenshots/scroll segments and metrics are in `logs/ui-feedback/compact-rules-v1215-qa-20260602`.
- Phase 32 follow-up fixes are implemented on 2026-05-28: scheduled Jellyfin auto-sync status now preserves detailed top-error text; duplicate/ambiguous Jellyfin matches such as multiple movie rows sharing IMDb `tt0317705` are skipped instead of counted as errors; Stremio watch-progress collection includes active no-progress library placeholders so Jellyfin-only progress such as `Hacks` `tt11815682:S05E09` can sync to Stremio; and shared sync top-error reporting no longer surfaces successful watch-progress messages as errors. Validation evidence: focused Jellyfin/Stremio/watch-progress regressions passed, full `cmd.exe /c scripts\check.bat` passed (`444 passed`, `290 warnings`), shared Docker Compose rebuild passed, Docker `/health` returned `status=ok` / `app_version=1.2.0`, live Docker `execute_jellyfin_sync(..., allow_metadata_requests=False)` returned no top errors, and live Docker `sync_watch_progress(...)` was idempotent with `matched=7`, `skipped=7`, `errors=0`.
- Phase 32 is implemented, automatic, and release-validated as `v1.2.0` (`docs/plans/phase-32-bidirectional-watch-progress-sync.md`): added bidirectional Stremio/Jellyfin watch-progress synchronization for movies and series episodes, including shared watch-progress records, newest-progress-wins arbitration with a 30-second minimum delta, Jellyfin HTTP API write-back settings, Stremio `datastorePut` write-back, a manual Settings action, automatic execution from existing Jellyfin/Stremio sync paths, and operation-progress instrumentation. Validation evidence: focused watch-state/orchestrator/Jellyfin/Stremio/settings/route regressions passed, full `cmd.exe /c scripts\check.bat` passed (`439 passed`, `290 warnings`), shared Docker Compose rebuild passed, Docker `/health` returned `status=ok` / `app_version=1.2.0`, and live Docker `execute_jellyfin_sync(..., allow_metadata_requests=False)` ran watch-progress sync automatically with `matched=3`, zero watch-progress errors, and no Jellyfin DB readability failure.
- Phase 32 same-series progress matching fix is implemented on 2026-05-27 after live sync reported `Jellyfin read: 19`, `Stremio read: 200`, but `matched: 0`: Jellyfin had `The Boys` progress at `tt1190634:S05E01`..`S05E05` while Stremio had only its series-level latest state at `tt1190634:S04E08`. Watch-progress sync now groups episode records by series IMDb key, picks the latest provider record per series, and synthesizes Stremio `video_id` values from Jellyfin winner keys such as `tt1190634:S05E05`.
- Phase 32 Stremio series write-shape fix is implemented and live-verified on 2026-05-27: episode progress writes no longer mark the whole Stremio series watched via `flaggedWatched` / `timesWatched`; instead they clear those series-level flags, set the current episode `video_id`, and update the exact `state.watched` episode bitfield. Stremio reads also ignore stale whole-series watched flags when deciding whether one episode is completed, and use per-episode `timeWatched` rather than series-level `overallTimeWatched` for episode progress. Jellyfin progress sync no longer treats `PlayCount=1` as completed when `Played=0`, because live Jellyfin data uses that shape for in-progress playback. Docker Jellyfin read reports `The Boys` S05E05 as `completed=False`, Docker sync is idempotent, and live Stremio bitfield verification shows S05E01-S05E04 true while in-progress S05E05 is false.
- Phase 31 is implemented and locally/Docker validated (`docs/plans/phase-31-shared-operation-progress-bar.md`): added a shared in-memory operation registry, `/api/operations/status`, producer instrumentation for qB sync, Jackett fetches, Jellyfin sync, and Stremio sync, plus a compact visible global progress bar in the base layout. The 2026-05-25 closeout bugs where the progress shell was hidden on idle/main-page loads, row sync opened the rule page, Sync All bypassed progress, and qB active-worker text could show the stale post-completion `2 active` snapshot are fixed and browser/test-verified. qB rule sync now uses bounded adaptive concurrency that starts at 3 workers, ramps up after healthy completion streaks, reports the active limit in progress text, halves the limit on timeout/transport/back-pressure-like failures, and performs feed refresh/reconciliation once per queued batch so workers do not repeat the same Jackett/qB feed setup per rule. Watch-progress write-back between Jellyfin/Stremio/qB Rules remains explicitly deferred to a later dedicated phase.
- Single-rule edit page action cleanup is implemented and validated on 2026-05-25: the two separate `Run Search Here` / `Refresh Search Snapshot` controls are now one snapshot-aware button that creates the first saved snapshot when none exists and refreshes the saved snapshot when one does; `Sync This Rule` on the edit page queues that one qB sync and returns to the same rule page while main-page row sync still returns to the workbench. Validation evidence: focused route regressions passed, `cmd.exe /c scripts\check.bat` passed with `417 passed` and `288 warnings`, shared Docker Compose rebuild passed, and Docker `/health` returned `status=ok` / `app_version=1.1.7` via `curl.exe` after PowerShell `Invoke-WebRequest` raised a local `NullReferenceException`.
- Search-result grouping/magnet/UI follow-up is implemented and release-validated as `v1.1.8`: Jackett same-release rows without an exposed infohash now bridge into a known-infohash sibling when title family, size bucket, year, and quality tags line up, so the Rick and Morty 78 GB multi-tracker pack collapses to one grouped row with four variants after refresh; search/rule results show a magnet-icon action that copies/opens one merged magnet from the shared hash and already-known tracker hints; and the wide rule edit page no longer produces document-level horizontal overflow in browser validation. Validation evidence: grouped hash-bridge regressions passed, `cmd.exe /c scripts\check.bat` passed (`418 passed`, `288 warnings`), desktop build passed, shared Docker Compose rebuild passed, Docker `/health` returned `status=ok` / `app_version=1.1.8`, and Playwright against rule `895b840e-5d41-49ba-9eff-79313cfa18bb` confirmed `scrollWidth=clientWidth=1600`, magnet buttons with SVG icons, and the target pack grouped with `duplicateCount=4`.
- Phase 31 qB drift-diagnostics patch is published as `v1.1.7`: successful qB rule repair now clears the active remote-drift banner/timestamp after `setRule` succeeds while preserving the last observed stale remote payload for diagnostics evidence. This fixes stale "next sync rewrites qB" copy after rules such as `Rick and Morty` have already been rewritten from local rule semantics.
...
- Current release `v1.3.3` is published; the next roadmap action is the normal discovery review.
...
- Implemented the Phase 32 bidirectional watch-progress sync foundation:
  - added shared provider-neutral watch-progress records and newest-progress-wins selection in `app.services.watch_state`;
  - added `app.services.watch_progress_sync` to collect provider progress, write the newer side to the older side, skip sub-30-second deltas, and report operation progress;
  - extended Jellyfin settings with server URL/API key write-back configuration while preserving existing read-only DB discovery;
  - added Jellyfin progress collection from `PlaybackPositionTicks` / `LastPlayedDate` and HTTP API write-back through `/Sessions/Playing/Progress`;
  - extended Stremio progress parsing from library item `state` and added `datastorePut` write-back that preserves existing item state fields;
  - exposed `POST /api/settings/sync-watch-progress` and a Settings page `Sync Watch Progress` action.
  - validation evidence: focused watch-state/orchestrator/Jellyfin/Stremio/settings/route tests passed, Ruff/mypy passed, full `cmd.exe /c scripts\check.bat` passed (`439 passed`, `290 warnings`), shared Docker Compose rebuild passed, Docker `/health` returned `status=ok` / `app_version=1.2.0`, and live Docker Jellyfin sync ran watch-progress sync automatically with zero watch-progress errors.
  - fixed the same-series mismatch found by real `The Boys` data: exact episode-key intersection has been replaced by series-level grouping for episode records, and Jellyfin-to-Stremio episode writes now set Stremio `video_id` from the winning `tt...:SxxEyy` key. Validation evidence: targeted regressions passed, live dry-run diagnostics found `matched=2` and `Updated Stremio from Jellyfin for tt1190634`, and full `cmd.exe /c scripts\check.bat` passed (`431 passed`, `290 warnings`).
  - fixed Stremio series write/read semantics so episode sync updates the watched-episode bitfield, clears stale whole-series watched flags, does not let those flags make a specific episode look completed, and explicitly clears the bit for an in-progress episode. The winning series group now writes all completed episodes plus the latest in-progress record. Jellyfin progress sync now uses `Played`, not `PlayCount`, for completed state. Validation evidence: targeted regressions passed (`3 passed` for the latest bug, `5 passed` for the broader series slice), full `cmd.exe /c scripts\check.bat` passed (`436 passed`, `290 warnings`), shared Docker Compose rebuild passed, Docker `/health` returned `status=ok`, and live Stremio verification shows `The Boys` S05E05 no longer marked watched.
  - wired watch-progress sync into existing Jellyfin/Stremio sync execution paths, so background auto-sync workers and `Save + Sync ... Now` actions run provider progress sync automatically when both providers are configured; manual `Sync Watch Progress` remains available for explicit one-off repair.
  - hardened Jellyfin SQLite reads by falling back to a read-only snapshot copy when the live DB is briefly unreadable through Docker/host mounts.
...
- Implemented the Phase 31 shared operation progress foundation:
  ...
  - fixed the main-page qB sync action regressions so individual rule sync queues background qB sync and returns to the rules workbench, Sync All queues all rules through the same progress-aware queue instead of the old blocking inline path, and queue progress refreshes after worker-slot refill so the active count reflects the intended three-worker cap;
  - replaced the fixed qB queue cap with bounded adaptive concurrency: it starts at three active rule syncs, grows by one after healthy completion streaks up to a 24-worker ceiling, backs off by halving on timeout/transport/back-pressure-like failures, and includes the learned limit in the visible progress message;
  - moved queued qB feed preparation out of every worker: the queue now refreshes rule language feeds and reconciles qB/Jackett RSS feeds once before dispatching a batch, then workers call `sync_rule(..., reconcile_feeds=False)` so parallel slots are not serialized by repeated feed setup; if batch preparation fails, workers fall back to per-rule reconciliation for correctness;
  - fixed stale qB drift diagnostics so a successful qB repair sync clears the active remote-drift message/timestamp after `setRule` succeeds while retaining the last observed remote payload evidence;
  - fixed the single-rule edit page snapshot/sync actions so the snapshot button is state-aware and edit-page qB sync returns to the same rule page after queueing; validation evidence: focused route regressions passed, full `scripts\check.bat` passed (`417 passed`, `288 warnings`), shared Docker Compose rebuild passed, and Docker `/health` returned `status=ok` / `app_version=1.1.7` via `curl.exe`;
  - added category/media compatibility filtering for saved rule snapshots so software/program archive rows such as a Windows 7 program image are hidden for movie/series rules instead of remaining visible only because a title fallback matched text;
  - validation evidence: operation registry/route/qB/Jackett/Jellyfin/Stremio/static tests passed, `node --check app/static/app.js` passed, touched-file Ruff checks passed through `.venv\Scripts\python.exe -m ruff`, `cmd.exe /c scripts\check.bat` passed (`415 passed`, `284 warnings`), desktop build passed with `0 Warning(s)` and `0 Error(s)`, shared Docker Compose rebuild passed, Docker `/health` returned `status=ok` / `app_version=1.1.7`, curl confirmed the main-page progress markup is served, Playwright confirmed the progress shell is visible on `/` with screenshot evidence at `logs/qa/phase-31-progress-visible/main-page-progress.png`, live Docker browser smoke confirmed Sync All changes the visible progress surface to `Background work running` with `Syncing qBittorrent rules`, and live Docker qB sync/readback for `Rick and Morty` cleared `drift_message` while matching the app-managed payload.
...
- Implemented Phase 27 structured audio search fields on 2026-05-07:
  ...
  - GitHub release state: commit `1011b723` is pushed to `main`, tag `v1.1.5` is pushed, and GitHub Release `https://github.com/Spon4ik/qBittorrent-rss-rules/releases/tag/v1.1.5` is published.
...
- Relaxed local hidden-row filtering for same-season complete packs on 2026-04-24 so saved-rule search can keep useful upgrade candidates visible when a series rule is set to continue searching after synced progress:
  ...
  - updated `app/services/jackett.py` so active Jackett searches for `jellyfin_search_existing_unseen` series rules derive the structured primary season/episode from the latest watched-progress episode instead of the stored known-episode floor; for example, the live `The Miniature Wife` rule now searches primary requests at `S01E02` instead of `S01E11`, allowing exact IMDb/title requests to find full `S1E1-10` upgrade packs;
  - added focused regressions in `tests/test_rule_fetch_ops.py` and `tests/test_routes.py`, and revalidated with `.\\.venv\\Scripts\\python.exe -m pytest tests\\test_rule_fetch_ops.py -k "complete_pack_when_keep_searching_enabled or zero_based_ranges_below_episode_floor" -q`, `.\\.venv\\Scripts\\python.exe -m pytest tests\\test_routes.py -k "inline_local_generated_pattern" -q`, and `.\\.venv\\Scripts\\python.exe -m pytest tests\\test_rule_builder.py -k "zero_based_ranges or keep_searching_existing_unseen" -q`.
  - added `tests/test_jackett.py::test_build_search_request_from_rule_uses_watched_progress_for_keep_searching_existing` and revalidated the search-payload update with `.\\.venv\\Scripts\\python.exe -m pytest tests\\test_jackett.py -k "watched_progress_for_keep_searching_existing or carries_series_episode_floor" -q`, `.\\.venv\\Scripts\\python.exe -m pytest tests\\test_routes.py -k "rule_search or inline_search or jackett_search_api or transform" -q`, and `.\\.venv\\Scripts\\python.exe -m ruff check app\\services\\jackett.py tests\\test_jackett.py`.

- Made qB rule sync failures visibly actionable on 2026-04-23:
  - updated `app/templates/index.html` so rules with `last_sync_status=error` now show the stored `last_sync_error` directly in both the table and card views instead of only displaying the small `error` chip;
  - added focused styling in `app/static/app.css` so long qB/Jacket failure text wraps in a visible inline error callout without breaking the rules table;
  - added `tests/test_routes.py::test_rules_page_renders_sync_error_details` and validated the rules-page slice with `.\\.venv\\Scripts\\python.exe -m pytest tests\\test_routes.py -k "rules_page" -q` plus `.\\.venv\\Scripts\\python.exe -m ruff check tests\\test_routes.py`.
...
- Added qB RSS rule language-driven feed selection on 2026-04-20 so rule authors can choose a language in the rule UI and let the app resolve matching Jackett-backed qB RSS feeds under the hood:
  ...
  - updated `app/routes/pages.py` again on 2026-04-22 so saved-rule active search no longer depends on live qB RSS feed availability when a rule is language-managed: rule search still prefers feed-derived Jackett scope when feeds are present, but now falls back to Jackett configured-indexer language metadata when feed URLs are missing or not Jackett-parseable, which preserves the intended Jackett-first active-search model while leaving passive RSS automation unchanged;
  - updated `app/routes/api.py` again on 2026-04-22 so editing an existing language-managed rule no longer fails just because qB RSS feeds are temporarily unavailable: the update path now preserves the rule's saved feed URLs when the language is unchanged, while new-rule creation still requires a real language-to-feed resolution for passive qB automation;
  - updated `app/routes/api.py`, `app/routes/pages.py`, and `tests/test_routes.py` again on 2026-04-22 so qB feed availability is now treated as a passive-RSS concern instead of a hard rule-save prerequisite: language-managed rules can now be created or updated while qB RSS feeds are unavailable, same-language edits preserve the previously resolved feed URLs, language changes save cleanly with an empty passive feed scope instead of blocking, and the edit form keeps saved feed URLs visible while warning that live qB feed resolution is currently unavailable;
  - added focused regressions in `tests/test_routes.py` that lock both the preserved feed-scope behavior and the new language-to-Jackett-indexer fallback for inline rule search plus the `/search?rule_id=...` workspace path;
...
- Phase 32 closeout validation is complete locally and in Docker; next follow-up is live provider smoke if real Jellyfin API/Stremio auth credentials are available.
...
- The current GitHub release is `v1.1.3`, adding the merged R1.5-R8 contract-roadmap guardrails and qB enforcement-parity diagnostics on top of the already tagged `v1.1.2` taxonomy/profile inheritance patch.
...
- Keep the merged Magnet action tied to known infohash/grouped tracker metadata only; avoid auto-fetching torrent files for magnet enrichment unless a later quota-aware cache/enrichment phase explicitly allows it.
...
- Keep the Office Space grouped-quality boundary regression whenever browser-side quality matching changes: short aliases such as `dv` must not match substrings such as `DVO` or `DVDRip`. Saved edit-page and rules-grid exact/visible/results counts must continue to use the same `Rule.last_*` snapshot summary projection.
