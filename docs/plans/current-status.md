...
- Implemented the Phase 31 shared operation progress foundation:
  ...
  - validation evidence: operation registry/route/qB/Jackett/Jellyfin/Stremio/static tests passed, `node --check app/static/app.js` passed, touched-file Ruff checks passed through `.venv\Scripts\python.exe -m ruff`, `cmd.exe /c scripts\check.bat` passed (`414 passed`, `284 warnings`), shared Docker Compose rebuild passed, Docker `/health` returned `status=ok` / `app_version=1.1.6`, curl confirmed the main-page progress markup is served, Playwright confirmed the progress shell is visible on `/` with screenshot evidence at `logs/qa/phase-31-progress-visible/main-page-progress.png`, and live Docker browser smoke confirmed Sync All changes the visible progress surface to `Background work running` with `Syncing qBittorrent rules`.
