...
- The `/search` form now exposes `IMDb ID`, `Release year`, and an explicit `Require IMDb ID` toggle; it first tries strict `imdbid`, then `q + imdbid`, and if the aggregate `all` indexer rejects both, it asks Jackett for configured indexer capabilities and retries direct indexers that actually support `imdbid`.
- Live Jackett capability inspection now confirms the current configured set advertises `movie-search imdbid` for some trackers but not `tv-search imdbid`; when that happens, IMDb-enforced series search now falls back to a title search and locally keeps only results whose returned Torznab metadata includes the matching `imdbid`.
...
- Manually verify `/rules/{rule_id}/search` with `Require IMDb ID` enabled and confirm `Requests used` shows strict `imdbid`, `q + imdbid`, direct-indexer `q + imdbid`, or a metadata-filtered title search tagged with `result_imdbid=...`, but never a successful flow that silently ignores the IMDb constraint.
