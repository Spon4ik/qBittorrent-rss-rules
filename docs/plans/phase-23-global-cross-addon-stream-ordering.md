...
- Historical note on 2026-04-19: the native Stremio addon ownership from this phase is now being retired from qBittorrent RSS Rules under phase 25, because the addon moved to `jackett-stremio-fork` and this repo now keeps only sync/watch-progress integration.
- A latest qB-side precision follow-up landed on 2026-04-18 after the user showed that rule `d434196a-c1b6-4202-a1ad-c62bc4294b43` (`Elsbeth`) still displayed wrong-season/wrong-episode season packs under `Precise results`: `app/services/stremio_addon.py` now reapplies the active primary Jackett payload filter before `collect_enriched_search_run(...)` promotes addon-baseline rows into the precise lane, so broad season packs such as `S3E1-15` are demoted back into fallback/debug instead of bypassing the stricter episode-floor matcher.
- A latest live queue-compatibility follow-up landed on 2026-04-18 after real end-to-end probing against rule `7af1ab04-85cb-468c-a0ec-9e3e3dbe7ec0` showed a saved BitRu `http://localhost:9117/dl/...` URL timing out for app-side download while qB on the same machine accepted the exact same URL immediately: `app/services/selective_queue.py` now allows loopback/private Jackett URLs to fall back to qB remote URL fetch when qB itself is also on loopback, while still keeping the older fail-closed behavior for non-local qB topologies.
- A latest qB WebUI compatibility follow-up landed on 2026-04-18 after the user hit `404 Not Found` on `http://localhost:8080/api/v2/torrents/filePrio`: `app/services/qbittorrent.py` now falls back to the legacy `/command/setFilePrio` endpoint when the modern file-priority route is missing, so grouped selective-download queue flows still work against older qB WebUI builds.
- A latest desktop-shell control follow-up landed on 2026-04-18 after the user asked for a real restart action in the UI rather than another script command: `QbRssRulesDesktop/Views/MainPage.xaml` now exposes `Restart Backend` beside the existing start/shutdown actions in both the header toolbar and offline panel, and `MainPage.xaml.cs` now stops the controllable local backend and immediately starts it again from the shell.
- A latest qB queue-hardening follow-up landed on 2026-04-18 after a live rule queue action still failed with "Could not fetch a valid torrent file" for a local Jackett-style URL that resolved to a magnet instead of raw `.torrent` bytes: `app/services/selective_queue.py` now probes loopback/private `dl/...` links for a direct `magnet:` redirect before rejecting them, keeping the earlier broken-local-URL safety contract while recovering queueable redirected-magnet results.
...
- A twenty-first phase-23 queue-hardening follow-up landed on 2026-04-18 after a live rule queue action still failed on a local Jackett-style `dl/...` result whose download path resolved to a magnet instead of torrent bytes:
  - updated `app/services/selective_queue.py` so loopback/private Jackett download URLs now probe for a non-HTTP `magnet:` redirect before the queue path declares the local fetch invalid, then hand that recovered magnet to qB through the existing magnet queue path;
  - kept the earlier broken-local-URL safeguard intact, so genuinely bad local Jackett URLs still stop in the app instead of falling through to qB remote URL fetch and creating repeated desktop add-failure notifications;
  - added focused regressions in `tests/test_selective_queue.py` and `tests/test_routes.py` proving redirected local Jackett magnets now queue successfully while the broken-local and dual-host rewrite behaviors remain locked in place;
  - revalidated with `.\\.venv\\Scripts\\python.exe -m pytest tests\\test_selective_queue.py -k "redirected_local_jackett_magnet or broken_local_jackett_url or rewrites_jackett_qb_url_for_app_fetch" -q`, `.\\.venv\\Scripts\\python.exe -m pytest tests\\test_routes.py -k "redirected_local_jackett_magnet or broken_local_jackett_url or rewrites_jackett_qb_url_for_app_fetch" -q`, and `.\\.venv\\Scripts\\python.exe -m ruff check app\\services\\selective_queue.py tests\\test_selective_queue.py tests\\test_routes.py`.
